...
// Enriquecer cada partido con: kickoff, línea O/U y momios over/under.
export const PARTIDOS = BASE.map(p => {
  const favMomio = Math.min(p.momios.local, p.momios.visita); // qué tan fuerte el favorito
  // Partidos muy disparejos tienden a más goles → línea 3.5; el resto 2.5
  const linea = favMomio <= 1.35 ? 3.5 : 2.5;
  const ouMomios = linea === 3.5 ? { over:2.05, under:1.75 } : { over:1.90, under:1.90 };
  return {
    ...p,
    kickoff: kickoffMs(p.fecha, p.hora),
    linea,
    momios: { ...p.momios, over: ouMomios.over, under: ouMomios.under },
  };
});
